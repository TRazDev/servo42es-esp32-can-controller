...
- **Stall test (user, 2026-09-11):** blocking the shaft made the motor **restart**, not trip stall protection. The position counter reset to ~0, and the stall flag was clear afterwards. Suspected cause: the lab PSU's 1.5 A current limit pulls the 24 V supply down under stall current, so the driver browns out (under 20 V). UNVERIFIED.
  ...
  - Added boot logging of the ESP32 settings. NVS works: keys are stored and loaded. A gear of 1.0 after a reboot means 1.0 was the last value saved.
- **Left to do:**
